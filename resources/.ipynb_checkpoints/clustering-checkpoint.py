...
class Clustering(flask_restful.Resource):
    def get(self, job_id):
        if job_id in jobs.keys():
            return jobs[job_id]
        return send_message("Processo não encontrado.")
    
    def post(self, job_id=None):
        data = flask.request.json
        uid = str(uuid.uuid4())
        thread = Thread(target = self.process_data, args=(data["messages"], uid,))
        thread.start()
        return uid
    
    def process_data(self, data, uid):
        print(data, uid)
        data = [phrase.replace("\n","") for phrase in data]
        ...
        cluster = KMeans(n_clusters=20 if len(data) > 20 else int(len(data)/2),random_state=0).fit(output.numpy())
        result = {}
        for i in set(cluster.labels_):
            ...
            result[int(i)] = self.show_first_n(data,i,cluster, nearest=nearest, most_frequent=",".join(most_freq))
        sem.acquire()
        jobs[uid] = result
        sem.release()
    ...
    def show_first_n(self,data,class_sample,cluster,n=10, nearest=None, most_frequent=None):
        result = [{"frase":data[i], "grupo":str(cluster.labels_[i]), "palavra_mais_proximos":nearest, "palavras_mais_proximas":most_frequent} for i in range(len(cluster.labels_)) if cluster.labels_[i] == class_sample][:n]
        return result